...
export const extractImageSources = (htmlString: string) => {
	const doc = new DOMParser().parseFromString(htmlString, 'text/html');
	const images = doc.querySelectorAll('img');
	return Array.from(images).map((img) => img.getAttribute('src'));
};

const getWeiboFirstImage = (doc: Document) => {
...
const findMediaEnclosure = (enclosures: Record<string, unknown>[]) => {
...
const findImageEnclosure = (enclosures: Record<string, unknown>[]) => {
	return enclosures?.find(
		(enclosure) =>
...
export const parseCoverImage = (entry: FeedEntry) => {
